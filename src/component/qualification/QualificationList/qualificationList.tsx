...
import { useGetQualificationsQuery, useCreateQualificationMutation, useUpdateQualificationMutation } from '../../../store/api/qualificationApi';

type Order = 'asc' | 'desc';

const QualificationList = () => {
    const { data: qualifications = [], refetch } = useGetQualificationsQuery();
    ...
    const [editData, setEditData] = useState<Qualification | null>(null);
    const [page, setPage] = useState(0);
    const [rowsPerPage, setRowsPerPage] = useState(10);
    const [orderBy, setOrderBy] = useState<keyof Qualification>('id');
    ...
    const handleSave = async (qualification: Qualification) => {
        try {
            if (qualification.id) {
                await updateQualification(qualification).unwrap();
            } else {
                await createQualification(qualification).unwrap();
            }
    ...
    const handleSort = (column: keyof Qualification) => {
                ...
                <TableContainer>
                    <Table size="small">
                        ...
                        <TableBody>
                            {paginated.length === 0 ? (
                                <TableRow>
                                    <TableCell colSpan={3} align="center">No qualifications found.</TableCell>
                                </TableRow>
                            ) : paginated.map(row => (
